...
util.inherits(Masm, jit.BaseMasm);

Masm.prototype.getHigh = function getHigh(src) {
  // Operand ([reg, offset])
...
Masm.prototype.getLow = function getLow(src) {
  // Operand ([reg, offset])
  if (Array.isArray(src)) {
    assert.equal(src.length, 2);
...
Masm.prototype.getIndex = function getIndex(src) {
...
// Emit rex.w prefix
Masm.prototype.rexw = function rexw(src, dst) {
  // rex.w
  var byte = 0x48;

  // rex.r
  if (src) byte |= this.getHigh(src) << 2;

  // rex.b
  if (dst) byte |= this.getHigh(dst);

  this.emitb(byte);
};

// Emit rex.w prefix only if operands have high bits
Masm.prototype.optrexw = function optrexw(src, dst) {
...
// Emit modrm byte
Masm.prototype.modrm = function modrm(src, dst) {
  var mod = 0,
      rm = 0,
      reg = 0,
      payload = 0,
  ...
  if (src) reg = typeof src === 'string' ? this.getLow(src) : src;
...
Masm.prototype.nop = function nop() {
  this.emitb(0x90);
};

Masm.prototype.push = function push(src) {
...
Masm.prototype.pop = function pop(dst) {
...
Masm.prototype.movq = function mov(src, dst) {
...
Masm.prototype.ret = function ret(count) {
  if (!count) {
    this.emitb(0xc3);
  } else {
    this.emitb(0xc2);
