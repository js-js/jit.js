...
Asm.prototype._mov = function _mov(size, dst, src) {
  ...
  if (typeof src === 'number') {
    ...
    assert.equal(size, 64, 'Buffer immediate supported only for mov');
...
// #### @dst {String|Array} General purpose register or memory address
// #### @src {String|Array|Number} General purpose register, memory
//                                 address, long number, or quad buffer
...
//
// ### function movw (dst, src)
// #### @dst {String|Array} General purpose register or memory address
// #### @src {String|Array|Number} General purpose register, memory
//                                 address, long number, or quad buffer
// Emit `movw` instruction, possible combinations:
// * `this.movw(regOrMem, number)`
// * `this.movw(regOrMem, reg)`
// * `this.movw(reg, regOrMem)`
//
Asm.prototype.movw = function movw(dst, src) {
  return this._mov(16, dst, src);
};

//
// ### function movb (dst, src)
// #### @dst {String|Array} General purpose register or memory address
// #### @src {String|Array|Number} General purpose register, memory
//                                 address, long number, or quad buffer
// Emit `movb` instruction, possible combinations:
// * `this.movb(regOrMem, number)`
// * `this.movb(regOrMem, reg)`
// * `this.movb(reg, regOrMem)`
//
Asm.prototype.movb = function movb(dst, src) {
  return this._mov(8, dst, src);
};
  ...
  return function binop(dst, src) {
    if (dst === 'rax' && typeof src === 'number') {
      if (this.isByte(src) && raxImmByte) {
        ...
        this.emitb(src);
        return;
      } else if (this.isByte(src)) {
        // Fall through to `src === 'number'`
      } else {
        this.rexw(null, dst);
        this.emitb(raxImm);
        this.emitl(src);
...
//
// ### function unOp (opcode, mode, size)
// #### @opcode {Number} Instruction opcode
// #### @mode {Number} Mode
// #### @size {Number} **Optional** input/output size (either 32 or 64)
// **internal** Generate generic unary operation instruction.
//
Asm.prototype._unOp = function unOp(opcode, mode, size) {
  return function unop(src) {
    this.rex(size || 64, null, src);
    this.emitb(opcode);
    this.modrm(mode, src);
  };
};
